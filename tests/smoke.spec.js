...
test('mail layout switches between ABC and AB reading modes', async ({ page }) => {
  ...
  const wideSelectionBar = mailView.locator('[data-mail-selection-bar="true"]');
  await expect(wideSelectionBar).toContainText('已选中 1 封邮件');
  const wideBatchActions = wideSelectionBar.locator('[data-mail-batch-actions="true"]');
  await expect(wideBatchActions).toBeVisible();
  await expect(wideBatchActions.getByRole('button', { name: '批量标为已读' })).toBeVisible();
  await expect(wideBatchActions.getByRole('button', { name: '批量标为未读' })).toBeVisible();
  await expect(wideBatchActions.getByRole('button', { name: '批量标记旗标' })).toBeVisible();
  await expect(wideBatchActions.getByRole('button', { name: '批量归档' })).toBeVisible();
  await expect(wideBatchActions.getByRole('button', { name: '批量导出' })).toBeVisible();
  await expect(wideBatchActions.getByRole('button', { name: '批量删除' })).toBeVisible();
  await expect(mailView.locator('[data-mail-selected-action-bar="true"]')).toHaveCount(0);
...
test('mail workspace uses a focused inbox layout', async ({ page }) => {
  ...
  await expect(bulkReader.getByRole('button', { name: '批量归档' })).toBeVisible();
